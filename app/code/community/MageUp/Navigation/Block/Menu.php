 ...
 */
class MageUp_Navigation_Block_Menu extends Mage_Core_Block_Template
{
    ...
    {
        /** @var $items MageUp_Navigation_Model_Resource_Menuitem_Collection */
        $items = Mage::getResourceModel('mageup_nav/menuitem_collection');
    ...
    public function renderItems(Varien_Data_Tree_Node_Collection $items)
    {
        ...
        foreach($items as $child) {
            $itemBlock = $this->getLayout()
                ->createBlock('mageup_nav/menu_item', 'item-'.$child->getId(), array('menu' => $this));
