 ...
 */
class MageUp_Navigation_Adminhtml_NavController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function categoriesJsonAction()
    {
        ...
        foreach ($category->getChildrenCategories() as $childCategory) {
            $children[] = Mage::getModel('mageup_nav/tree')->getTreeNode($childCategory);
        }
    ...
    public function cmsJsonAction()
    {
        ...
        foreach ($collection as $page) {
            ...
            $children[] = Mage::getModel('mageup_nav/tree')->getTreeNode($page);
        }
        $this->getResponse()->setBody(
            Zend_Json::encode($children)
        );
    ...
    public function getMenuItemsAction()
    {
        ...
        $collection = Mage::getResourceModel('mageup_nav/menuitem_collection')->addParentFilter($id)->sort();
        $children = array();
        foreach ($collection as $child) {
            $children[] = Mage::getModel('mageup_nav/tree')->getTreeNode($child);
        }
        $this->getResponse()->setBody(
            Zend_Json::encode($children)
        );
    ...
    public function syncMenuItemAction()
    {
        ...
        if (!empty($requestBody)) {
            ...
            if (isset($data['text'])) {
                // single item array
                /** @var $item MageUp_Navigation_Model_Menuitem */
                $item = Mage::getModel('mageup_nav/menuitem');
                if ($data['id']) {
                    $item->load($data['id']);
                }
                try {
                    if ($isDelete) {
                        ...
                        $item->addData($data)
                            ->map()
                            ->save();
                        $returnData->setChildren(array(Mage::getModel('mageup_nav/tree')->getTreeNode($item)));
                    }
                } catch (Exception $e) {
                ...
                try {
                    ...
                    foreach ($items as $i) {
                        if ($isDelete) {
                            ...
                            $i->save();
                            $updatedItems[] = Mage::getModel('mageup_nav/tree')->getTreeNode($i);
                        }
                    }
